-- stg_orders.sql
-- Staging model: cleaned and typed Olist orders from the Redshift raw layer.
...
cleaned as (
    ...
    select
        ...
        cast(order_purchase_year            as bigint)    as order_purchase_year,
        cast(order_purchase_month           as bigint)    as order_purchase_month,
        cast(order_delivered_customer_date  as timestamp) as order_delivered_customer_date,
        cast(order_estimated_delivery_date  as timestamp) as order_estimated_delivery_date,
        cast(delivery_days                  as float8)    as delivery_days,
        cast(estimated_delivery_days        as float8)    as estimated_delivery_days,
        ...
        cast(freight_value                  as float8)    as freight_value,
        cast(total_item_value               as float8)    as total_item_value,
        cast(payment_type                   as varchar)   as payment_type,
        cast(payment_value                  as float8)    as payment_value,
        cast(review_score                   as float8)    as review_score
